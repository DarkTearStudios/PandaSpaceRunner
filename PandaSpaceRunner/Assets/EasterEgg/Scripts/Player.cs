...
public class Player : MonoBehaviour {
	...
	void Update () {
		...
		if ((transform.position.y > getMaxWidth() || transform.position.y < -getMaxWidth() ) && GameManager2.instance.currentState == GameStates.INGAME) {
			Dead();
		}
			
		//When click or touch and in INGAME states
		if ((Input.GetMouseButtonDown(0) || Input.GetKeyDown(KeyCode.Space)) && GameManager2.instance.currentState == GameStates.INGAME){
			Jump();
		}

		//When click or touch and in PREGAME states
		if ((Input.GetMouseButtonDown(0) || Input.GetKeyDown(KeyCode.Space)) && GameManager2.instance.currentState == GameStates.PREGAME){
	...
	{
		if (other.transform.tag == "Obstacle" && GameManager2.instance.currentState == GameStates.INGAME) {
			Dead ();
		}
	}

	void OnTriggerEnter2D(Collider2D other){
		if (other.tag == "Score" && GameManager2.instance.currentState == GameStates.INGAME) {
